  ...
  <div v-if="visible" class="modal-overlay" @click.self="close">
    <div class="modal-container accounting-modal">
        ...
        <div v-else class="accounting-section">
          ...
          <div v-if="activeTab === 'stock'" class="stock-tab">
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  ><select v-model="newItem.type">
                    ...
                    <option v-for="t in filterTags" :key="t" :value="t">
                      {{ t }}
                    </option>
                  </select>
                </div>
                ...
                <button @click="addStockItem" class="add-btn">
                  ➕ Добавить
                </button>
              </div>
            </div>
            ...
            <div class="filter-tags">
              <button
                ...
                @click="
                  activeTypeFilter = activeTypeFilter === tag ? '' : tag;
                  showEmptyOnly = false;
                "
              >
                {{ tag }}
              </button>
              <!-- Отдельный фильтр "Требует пополнения" -->
              <button
                class="tag-btn"
          ...
          <div v-if="activeTab === 'treasury'" class="treasury-tab">
              ...
              <div class="action-card income-card">
                ...
                <div class="action-fields">
                  ...
                  /><input
                    ...
                    class="note-input"
                  /><button @click="addIncome" class="action-btn income-btn">
                    ➕ Добавить
                  </button>
                </div>
              </div>
              <div class="action-card expense-card">
                ...
                <div class="action-fields">
                  ...
                  /><input
                    ...
                    placeholder="Комментарий"
                    class="note-input"
                  /><button @click="addExpense" class="action-btn expense-btn">
                    ➖ Списать
                  </button>
                </div>
              </div>
            ...
            <div class="history-table-container">
              <table class="history-table">
                ...
                <tbody>
                  <tr
                    v-for="op in treasury.history
                      .slice()
                      .reverse()
                      .slice(0, 20)"
                    :key="op.date + op.amount + op.note"
                  >
          ...
          <div v-if="activeTab === 'chart'" class="chart-tab">
            <div class="section-title-small">
              📊 Количество проданных единиц по предметам
            </div>
      ...
      <div class="modal-footer">
        ...
        ><button v-if="authenticated" class="btn-refresh" @click="refreshData">
          🔄 Обновить
        </button>
      </div>
    </div>
    ...
    >
      <div class="sell-modal">
        ...
        <div class="sell-body">
          <div v-if="!sellDialog.loading">
            ...
            <div class="sell-field">
              ...
              ><input
                type="number"
                v-model.number="sellDialog.amount"
                min="0"
                step="any"
              />
            </div>
            <div class="sell-error" v-if="sellDialog.error">
              {{ sellDialog.error }}
            </div>
          </div>
        ...
        <div class="sell-footer" v-if="!sellDialog.loading">
          ...
          ><button @click="confirmSell" class="btn-sell">
            ✅ Подтвердить продажу
          </button>
        </div>
      </div>
      ...
      <div class="sell-modal">
        ...
        <div class="sell-body">
          <div v-if="!removeDialog.loading">
            <div class="sell-field">
              <label
                >Количество для удаления (макс.
                {{ removeDialog.maxQuantity }}):</label
              ><input
                type="number"
            ...
            <div class="sell-error" v-if="removeDialog.error">
              {{ removeDialog.error }}
            </div>
          </div>
          <div v-else class="sell-loading">
    ...
    <!-- Модалка увеличения (+) -->
    <div
      v-if="increaseDialog.visible"
      class="modal-overlay nested"
      @click.self="closeIncreaseDialog"
    >
      <div class="sell-modal">
        <div class="sell-header">
          📈 Увеличить: {{ increaseDialog.itemName }}
        </div>
        <div class="sell-body">
          <div v-if="!increaseDialog.loading">
            ...
            <div class="sell-error" v-if="increaseDialog.error">
              {{ increaseDialog.error }}
            </div>
          </div>
          <div v-else class="sell-loading">
            <div class="spinner"></div>
        ...
        <div class="sell-footer" v-if="!increaseDialog.loading">
          ...
          ><button @click="confirmIncrease" class="btn-sell">
            ✅ Добавить
          </button>
        </div>
      </div>
    ...
    <!-- Модалка уменьшения (-) -->
    <div
      v-if="decreaseDialog.visible"
      class="modal-overlay nested"
      @click.self="closeDecreaseDialog"
    >
      <div class="sell-modal">
        <div class="sell-header">
          📉 Уменьшить: {{ decreaseDialog.itemName }}
        </div>
        <div class="sell-body">
          <div v-if="!decreaseDialog.loading">
            <div class="sell-field">
              <label
                >Количество для списания (макс.
                {{ decreaseDialog.maxQuantity }}):</label
              ><input
                type="number"
            ...
            <div class="sell-error" v-if="decreaseDialog.error">
              {{ decreaseDialog.error }}
            </div>
          </div>
          <div v-else class="sell-loading">
            <div class="spinner"></div>
        ...
        <div class="sell-footer" v-if="!decreaseDialog.loading">
          ...
          ><button @click="confirmDecrease" class="btn-remove">
            ➖ Списать
          </button>
        </div>
      </div>
...
const API_URL =
  "https://script.google.com/macros/s/AKfycbzpt6QII0qaENioQXWCwCpEW5b78qFLqKkH1nkUo5AYbI-97ylOztf6uE3lwFYQ09VioA/exec";
...
const filterTags = ["Оружие", "Расходник", "Ресурс"]; // без "Пустые"
const activeTypeFilter = ref("");
const showEmptyOnly = ref(false); // новый флаг для фильтра "Требует пополнения"
...
const chartData = computed(() => {
  const itemsWithSales = stockItems.value.filter(
    (item) => item.sold_qty && item.sold_qty > 0,
  );
...
function checkPassword() {
  if (password.value === PASSWORD) {
    ...
    autoUpdateInterval = setInterval(() => {
      if (authenticated.value) fetchAccountingData();
    }, 30000);
  } else {
...
async function confirmSell() {
  ...
  sellDialog.value.error = "";
  try {
    await callApi("sellItem", {
      id: sellDialog.value.itemId,
      quantity: qty,
      amount: amt,
    });
    await fetchAccountingData();
...
async function confirmRemove() {
  ...
  removeDialog.value.error = "";
  try {
    await callApi("removeItem", {
      id: removeDialog.value.itemId,
      quantity: qty,
    });
    await fetchAccountingData();
    closeRemoveDialog();
...
async function confirmDecrease() {
  ...
  try {
    await callApi("removeItem", {
      id: decreaseDialog.value.itemId,
      quantity: qty,
    });
    await fetchAccountingData();
    closeDecreaseDialog();
...
const filteredStock = computed(() => {
  ...
  if (showEmptyOnly.value) {
    ...
    // Обычный режим: только позиции с количеством > 0
    filtered = filtered.filter((item) => item.quantity > 0);
    // Если выбран конкретный тип, фильтруем по нему
    if (activeTypeFilter.value) {
      filtered = filtered.filter(
        (item) => item.tape === activeTypeFilter.value,
      );
    }
  }

  // Поиск по названию работает всегда
...
.treasury-summary {
  background: linear-gradient(135deg, #2a241f, #1e1a16);
  border-radius: 12px;
  padding: 20px;
  text-align: center;
  margin-bottom: 20px;
...
.total-sum-value {
  ...
  font-family: "Cinzel", serif;
  color: #e8a870;
  text-shadow: 0 0 8px rgba(200, 120, 50, 0.3);
}
.treasury-actions {
  display: flex;
  gap: 20px;
...
.tag-btn.active {
  background: #8b2500;
  box-shadow: 0 0 4px #c4713b;
}
/* Стиль для кнопки "Требует пополнения" */
.empty-filter {
  background: #4a3a5a;
  margin-left: 12px;
}
